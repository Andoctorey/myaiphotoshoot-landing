...
const pageNumberClassName = 'inline-flex h-10 min-w-10 items-center justify-center rounded-lg px-3 font-semibold';
...
export default async function AiPresetsIndex({ locale, page = 1, pageData }: Props) {
  ...
  const pageNumbers = Array.from({ length: presetsPage.totalPages }, (_, index) => index + 1);
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              ...
              <div className="flex items-center gap-1">
                {pageNumbers.map((pageNumber) => {
                  const pageLabel = t('paginationPageTitle', { page: pageNumber });

                  return pageNumber === presetsPage.page ? (
                    <span
                      key={pageNumber}
                      aria-current="page"
